...
public class ModificarPersonaUseCase(IServicioAutorizacion auth, IRepositorioPersona repo, PersonaModificacionValidador validador, IRepositorioUsuario repoU)
{
    public void Ejecutar(Entities.Persona? persona, int unId)
    {
        if (!auth.PoseeElPermiso(repoU.ObtenerUsuarioPorId(unId), Permiso.UsuarioModificacion))
