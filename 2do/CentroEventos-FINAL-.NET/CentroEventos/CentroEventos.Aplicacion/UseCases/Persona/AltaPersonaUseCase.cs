...
public class AltaPersonaUseCase(IServicioAutorizacion auth, IRepositorioPersona repo, PersonaValidador validadorPersona, EmailValidador validarEmail, DniValidador validarDni, IRepositorioUsuario repoU) 

{  
    public void Ejecutar(Entities.Persona? persona, int unId)
    {
        if (!auth.PoseeElPermiso(repoU.ObtenerUsuarioPorId(unId), Permiso.UsuarioAlta)) throw new FalloAutorizacionException("No posee el permiso para hacer alta de Persona.");
        string msg;
