...
public class BajaPersonaUseCase(IServicioAutorizacion auth, IRepositorioPersona repo,IRepositorioEventoDeportivo repoEventoDeportivo,IRepositorioReserva repoReserva,PersonaBajaValidador validador,IRepositorioUsuario repoU)
{
    public void Ejecutar(int personaId, int unId)
    {
        if (!auth.PoseeElPermiso(repoU.ObtenerUsuarioPorId(unId), Permiso.UsuarioBaja))
