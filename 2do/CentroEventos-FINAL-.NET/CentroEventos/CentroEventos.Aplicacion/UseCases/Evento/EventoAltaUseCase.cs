...
public class EventoAltaUseCase(IServicioAutorizacion auth,
 ...
 EventoAltaValidadorDesc vDesc, EventoAltaValidadorDuracion vDur, IRepositorioUsuario repoU)
{
    public void Ejecutar(EventoDeportivo evento, int userId) 
    {
        if (!auth.PoseeElPermiso(repoU.ObtenerUsuarioPorId(userId), Permiso.EventoAlta))
        {
