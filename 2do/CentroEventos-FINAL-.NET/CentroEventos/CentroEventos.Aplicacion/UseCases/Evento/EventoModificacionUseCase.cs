...
IRepositorioEventoDeportivo repoEven, IRepositorioReserva repoR, IRepositorioPersona repoPer,
 ...
 EventoModificadorValidadorIdResponsable vId, IRepositorioUsuario repoU) {

    public void Ejecutar(EventoDeportivo unEvento, int userId) 
    {
        if (auth.PoseeElPermiso(repoU.ObtenerUsuarioPorId(userId), Permiso.EventoModificacion))
        {
